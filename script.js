...
function exportarWord() {
  ...
  a.href = url
  a.download = `${cargo.replace(/\s+/g, "_").toLowerCase()}_${nivel}.doc`
  a.click()
  URL.revokeObjectURL(url)
...
function abrirPesquisaSalarial(id) {
  ...
  document.getElementById("pesq-val-usina").value = p.val_usina || ""

  document.getElementById("btn-export-pesq").style.display = ""
  atualizarDashboardSalarial()
...
function atualizarDashboardSalarial() {
  const salMed = Number(document.getElementById("pesq-sal-med").value) || 0
  const valUsina = Number(document.getElementById("pesq-val-usina").value) || 0

  if (!salMed) return

  // Simular variacao entre fontes
  ...
  document.getElementById("chart-portal-val").textContent    = fmt(portal)
  document.getElementById("chart-media-val").textContent     = fmt(media)

  // Calcular percentuais para o donut (Glassdoor 50% + Dissidio 50% = pesos reais)
  const total = glassdoor + dissidio + caged + portal
  const pG = (glassdoor / total) * 360
  const pD = (dissidio / total) * 360
  const pC = (caged / total) * 360
  // Portal pega o restante

  document.getElementById("pesq-donut").style.background =
    `conic-gradient(#3b82f6 0deg ${pG}deg, #8b5cf6 ${pG}deg ${pG + pD}deg, #f59e0b ${pG + pD}deg ${pG + pD + pC}deg, #ec4899 ${pG + pD + pC}deg 360deg)`

  // Range bar min-med-max
  const salMin = Number(document.getElementById("pesq-sal-min").value) || Math.round(salMed * 0.82)
  const salMax = Number(document.getElementById("pesq-sal-max").value) || Math.round(salMed * 1.18)
  const rangeBar = document.getElementById("pesq-range-bar")
  rangeBar.style.display = "block"

  const span = salMax - salMin || 1
  const medPct = ((media - salMin) / span) * 100
  document.getElementById("pesq-range-fill").style.left = "0%"
  document.getElementById("pesq-range-fill").style.width = "100%"
  document.getElementById("pesq-range-marker-med").style.left = Math.min(Math.max(medPct, 5), 95) + "%"
  document.getElementById("pesq-range-min").textContent = fmt(salMin)
  document.getElementById("pesq-range-med").textContent = fmt(media)
  document.getElementById("pesq-range-max").textContent = fmt(salMax)

  // Usina
  const usinaRow = document.getElementById("chart-usina-row")
  const usinaMarker = document.getElementById("pesq-range-marker-usina")
  if (valUsina) {
    usinaRow.style.display = "block"
    document.getElementById("chart-usina-val").textContent = fmt(valUsina)

    // Marker na range bar
    const usinaPct = ((valUsina - salMin) / span) * 100
    usinaMarker.style.display = "block"
    usinaMarker.style.left = Math.min(Math.max(usinaPct, 3), 97) + "%"
    ...
    if (valUsina < media * 0.90) {
      ...
      posEl.innerHTML = `<div style="display:flex;align-items:center;gap:6px;margin-bottom:4px"><i class="uil uil-arrow-down" style="font-size:14px"></i><strong>Abaixo do mercado (${diff}%)</strong></div>O valor praticado esta abaixo da media de mercado para o setor sucroenergetico na regiao. Considere avaliar a competitividade salarial para retencao de talentos.`
      ...
      posEl.innerHTML = `<div style="display:flex;align-items:center;gap:6px;margin-bottom:4px"><i class="uil uil-arrow-up" style="font-size:14px"></i><strong>Acima do mercado (+${diff}%)</strong></div>O valor praticado esta acima da media regional. A usina tem boa competitividade salarial para atracao e retencao.`
    } else {
      posEl.style.background = "rgba(59,130,246,0.06)"
      posEl.style.border = "1px solid rgba(59,130,246,0.15)"
      posEl.style.color = "#2563eb"
      posEl.innerHTML = `<div style="display:flex;align-items:center;gap:6px;margin-bottom:4px"><i class="uil uil-check-circle" style="font-size:14px"></i><strong>Na media do mercado (${diff > 0 ? "+" : ""}${diff}%)</strong></div>O valor praticado esta alinhado com a media de mercado para o setor sucroenergetico no Sul Goiano.`
    }
  } else {
    usinaRow.style.display = "none"
    usinaMarker.style.display = "none"
    document.getElementById("pesq-posicionamento").style.display = "none"
  }

  // Texto explicativo
  const cargo = document.getElementById("pesq-cargo").value.trim() || "este cargo"
  const area = document.getElementById("pesq-area")
  const areaLabel = area.options[area.selectedIndex]?.text || area.value || "area"
  const nivel = document.getElementById("pesq-nivel").value || "Pleno"
...
function exportarPesquisaPDF() {
  if (!pesquisaEditando) { showToast("Salve a pesquisa antes de exportar.", "error"); return }
  window.open("/exportar/salario-pdf/" + pesquisaEditando, "_blank")
}
