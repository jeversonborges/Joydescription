...
// ═══════════════════════════════════════════════════════════════
//  DISSÍDIO — scraping real (média nacional setor sucroalcooleiro)
// ═══════════════════════════════════════════════════════════════

const dissidioCache = new Map()

function cargoParaDissidioSlug(cargo) {
  const norm = cargo.toLowerCase()
    .normalize("NFD").replace(/[\u0300-\u036f]/g, "")
    .trim()
    .replace(/[^a-z0-9\s]/g, " ")
    .replace(/\s+/g, "-")
  ...
  // Slugs específicos do setor sucroalcooleiro no Dissídio
  const sugarMap = [
    [["tecnico quimico", "tecnico em acucar", "tecnico acucar", "tecnico alcool"], "tecnico-em-acucar-e-alcool"],
    [["engenheiro quimico acucar", "engenheiro quimico alcool"], "engenheiro-quimico-acucar-e-alcool"],
    [["operador de moenda", "operador moenda"], "operador-de-moenda-na-fabricacao-de-acucar"],
    [["operador de secador"], "operador-de-secador-na-fabricacao-e-refino-de-acucar"],
    [["operador de cristalizacao", "operador cristalizacao"], "operador-de-cristalizacao-na-refinacao-de-acucar"],
    [["operador de cozedor", "operador cozedor"], "operador-de-cozedor-a-vacuo-na-refinacao-de-acucar"],
    [["operador de tratamento de calda"], "operador-de-tratamento-de-calda-na-refinacao-de-acucar"],
    [["operador de producao", "operador producao"], "operador-de-producao"],
    [["mecanico industrial", "mecanico"], "mecanico-industrial"],
    [["eletricista industrial", "eletricista"], "eletricista-industrial"],
    [["instrumentista"], "instrumentista"],
    [["soldador"], "soldador"],
    [["laboratorista"], "laboratorista-industrial"],
    [["analista de qualidade", "analista qualidade"], "analista-de-qualidade"],
    [["tecnico agricola", "tecnico em agronomia"], "tecnico-agricola"],
    [["engenheiro agronomo", "engenheiro agronomo"], "engenheiro-agronomo"],
    [["analista de rh", "analista rh", "analista de recursos humanos"], "analista-de-recursos-humanos"],
    [["tecnico de seguranca", "tecnico seguranca do trabalho"], "tecnico-de-seguranca-do-trabalho"],
    [["analista financeiro"], "analista-financeiro"],
    [["analista contabil", "analista de contabilidade"], "analista-contabil"],
    [["assistente administrativo"], "assistente-administrativo"],
    [["analista de ti", "analista ti"], "analista-de-tecnologia-da-informacao"],
    [["motorista"], "motorista"],
    [["supervisor de producao", "supervisor producao"], "supervisor-de-producao"],
  ]

  for (const [palavras, slug] of sugarMap) {
    if (palavras.some(p => norm.includes(p.replace(/\s/g, "-")))) return slug
  }
  return norm
}

async function buscarDissidio(cargo) {
  const slug = cargoParaDissidioSlug(cargo)
  const cached = dissidioCache.get(slug)
  if (cached && Date.now() - cached.ts < 86400000) return cached.data

  try {
    const url = `https://dissidio.com.br/salario/${slug}/`
    ...
    if (!resp.ok) { dissidioCache.set(slug, { data: null, ts: Date.now() }); return null }
    ...
    const parseBR = v => {
      ...
      return (n >= 1200 && n <= 50000) ? Math.round(n) : null
    }

    // Tentar extrair valor específico do setor sucroalcooleiro
    let salMed = null
    let descricao = "Média nacional — setor sucroalcooleiro"

    const padroesSugar = [
      /[Ff]abrica[çc][aã]o de [aá]lcool[^<\n]*?([\d]{1,3}(?:\.\d{3})*,\d{2})/,
      /[Ff]abrica[çc][aã]o de [Aa][çc][úu]car[^<\n]*?([\d]{1,3}(?:\.\d{3})*,\d{2})/,
      /sucro[aá]lcool[^<\n]*?([\d]{1,3}(?:\.\d{3})*,\d{2})/i,
      /[Ff]abrica[çc][aã]o de [aá]lcool[\s\S]{0,80}?([\d]{1,3}(?:\.\d{3})*,\d{2})/,
    ]
    for (const p of padroesSugar) {
      const m = html.match(p)
      if (m) { salMed = parseBR(m[1]); if (salMed) break }
    }

    // Fallback: qualquer média nacional da página
    if (!salMed) {
      descricao = "Média nacional — ocupação geral"
      const padroesGeral = [
        /sal[aá]rio m[eé]dio[\s\S]{0,60}?([\d]{1,3}(?:\.\d{3})*,\d{2})/i,
        /m[eé]dia\s+nacional[\s\S]{0,60}?([\d]{1,3}(?:\.\d{3})*,\d{2})/i,
      ]
      for (const p of padroesGeral) {
        const m = html.match(p)
        if (m) { salMed = parseBR(m[1]); if (salMed) break }
      }
    }

    if (!salMed) { dissidioCache.set(slug, { data: null, ts: Date.now() }); return null }

    const resultado = {
      sal_min: Math.round(salMed * 0.82),
      sal_med: salMed,
      sal_max: Math.round(salMed * 1.18),
      descricao,
      url: `https://dissidio.com.br/salario/${slug}/`,
    }
    dissidioCache.set(slug, { data: resultado, ts: Date.now() })
    console.log(`✅ Dissídio: "${cargo}" → R$ ${salMed} (${descricao})`)
    return resultado

  } catch (e) {
    console.warn(`⚠️ Dissídio falhou para "${cargo}":`, e.message)
    dissidioCache.set(slug, { data: null, ts: Date.now() })
    return null
  }
}

app.get("/dissidio-salarial", async (req, res) => {
  const { cargo } = req.query
  if (!cargo?.trim()) return res.status(400).json({ erro: "Parâmetro 'cargo' obrigatório." })
  const dados = await buscarDissidio(cargo.trim())
  res.json(dados || null)
})
